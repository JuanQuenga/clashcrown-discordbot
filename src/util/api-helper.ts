...
import { IApiCards } from '../config/interfaces/cards';
import { IApiClanLeaderboard } from '../config/interfaces/clan-leaderboard';
import { IApiClanProfile } from '../config/interfaces/clan-profile';
import { IApiLocation, IApiLocations } from '../config/interfaces/locations';
import { IApiPlayersBattleLog } from '../config/interfaces/player-battle-logs';
import { IApiPlayerLeaderboard } from '../config/interfaces/player-leaderboard';
import { IApiPlayerProfile } from '../config/interfaces/player-profile';
import { IApiPlayersUpcomingChests } from '../config/interfaces/player-upcoming-chests';
...
export class CRApi {
  ...
  constructor(uri: string, token: string, options?: IApiOptions) {
    ...
    this.apiRequest = Axios.create({
      ...
      headers: {
        ...
        'User-Agent': 'Clash Royale Stats Bot'
      },
      responseType: 'json'
    });
  }
   ...
   * @param playerTag Tag of the player to retrieve.
   */
  public async verifyToken(apiToken: string, playerTag: string): Promise<IVerifyTokenResponse> {
    ...
    return <IVerifyTokenResponse> (await request(route)).data;
  }
